...
/**
 * The symbol table class. Each symbol table contains a Vector that contains the symbols defined in the scope that it
 * corresponds to, and a reference to the symbol table for its enclosing scope (if any) or to the closest import
 * statement going backwards. This reference is the 'parent' and is used for name resolution. A second reference might
 * be needed for imports that themselves import other files/libraries in order to name check correctly; however, these
 * symbol tables cannot be used for looking up names in the main program, so they are kept separate in an 'importParent'
 * field.
 * 
 */
        ...
        for (; col.hasMoreElements();) {
            ...
            System.out
                    .print(indent
                            ...
                            + "\t"
                            + (o.getClass().getName()
                                    .equals("Utilities.SymbolTable") ? "Procedure: "
                                    : o.getClass().getName()));
