...
public class AllocateStackSize extends Visitor<Void>{
    ...
    private final String concatStrings = "__concatStrings";
    private final String externalCallN = "ExternalCallN";
    ...
    //========================================================================================
    /**
     * If we hit a binary expr we will catch the special case where it string concatanation
     * as this requires a function call.
     */
    public Void visitBinaryExpr(BinaryExpr be){
        Log.log(be.line + ": Visiting a BinaryExpr!");
        be.visitChildren(this);

        //Only consider string concatanation.
        boolean stringConcat = (BinaryExpr.PLUS == be.op()) &&
            (be.type instanceof PrimitiveType) &&
            ((PrimitiveType) be.type).isStringType();
        ...
        //This was a string concatanation! Allocate memory. Notie
        size1 = suTable.get(concatStrings);
        size2 = suTable.get(externalCallN);
        tag = "BinaryExpr: Call to __concatStrings function and ExternalCallN inside..";
        nodesPerFunction.addNode(currentFunction, new ValueAndTag(size1 + size2, tag, true));

        return null;
    }
    //========================================================================================
    /**
     * Special case where we have a string literal assignment. Then we must call strcpy
     * through an ExternalCallN.
     */
    public Void visitAssignment(Assignment as){
        Log.log(as.line + ": Visiting a Assignment!");
        ...
        if (as.right() instanceof NewArray){
            //Call to Malloc.
            size = suTable.get(malloc);
            tag = "VisitAssignment: Call to MAlloc function.";
            nodesPerFunction.addNode(currentFunction, new ValueAndTag(size, tag, true));
        }

        //Handle String.
        boolean stringLiteral = as.type instanceof PrimitiveType &&
            ((PrimitiveType) as.type).isStringType() &&
            as.right() instanceof PrimitiveLiteral;

        if(stringLiteral){
            //Handle case where we use strcpy for a string literal!
            //This was a string copy! Allocate memory.
            size = suTable.get(externalCallN);
            tag = "BinaryExpr: Call to ExternalCallN function.";
            nodesPerFunction.addNode(currentFunction, new ValueAndTag(size, tag, true));
        }
    ...
    //========================================================================================
    /** New arrays call MAlloc for memory allocation! */
    public Void visitNewArray(NewArray ne){

        Log.log(ne.line + ": Visiting a NewArray!");
        int size = suTable.get(malloc);
        String tag = "Call to MAlloc for new array allocation!";
        nodesPerFunction.addNode(currentFunction, new ValueAndTag(size, tag, true));
