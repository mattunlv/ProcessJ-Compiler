...
public class TypeChecker extends Visitor<Type> {
	...
	public Type visitRecordAccess(RecordAccess ra) {
		...
		if (tType.isArrayType() && ra.field().getname().equals("size")) {
            ...
            if (!tType.isRecordType()) {
				ra.type = Error.addError(ra,"Request for member '" + ra.field().getname() + "' in something not a record type.", 0);
				return ra.type;
			}				
			...
			if (rm == null) {
				ra.type = Error.addError(ra,"Record type '" + ((RecordTypeDecl)tType).name().getname() + "' has no member '" + ra.field().getname() + "'.", 00);
				return ra.type;
			}	
	...
	public Type visitSwitchStat(SwitchStat ss) {
		...
		if (!eType.isIntegralType() || eType.isLongType())
			Error.error(ss, "Switch statement expects value of type int.", false, 0);
		...
		for (SwitchGroup sg : ss.switchBlocks() ) {
			...
			if (!lType.isIntegralType() || lType.isLongType())
			    Error.error(sl, "Switch labels must be of type int.", false, 0);
			if (!sl.expr().isConstant())
			    Error.error(sl, "Switch labels must be constants.", false, 0);
		    }
