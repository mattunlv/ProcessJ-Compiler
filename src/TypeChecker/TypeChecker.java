...
public class TypeChecker extends Visitor<Type> {
	...
	*/
	...
	if (tType.isArrayType() && ra.field().getname().equals("size")) {
		...
		ra.type = Error.addError(ra,"Request for member '" + ra.field().getname() + "' in something not a record orprotocol type.", 3061);
		return ra.type;
	    }
		...
		if (rm == null) {
		    ra.type = Error.addError(ra,"Record type '" + ((RecordTypeDecl)tType).name().getname() + "' has no member '" + ra.field().getname() + "'.", 3062);
		    return ra.type;
		}	
		...
		*/
		...
		for (RecordMember rm : pc.body()) {
		    ...
		    if (rm.name().getname().equals(fieldName)) {
		    	System.out.println("yep we found it......now set it.");
		...
		if (!found) {
		    Error.error(ra, "Unknown field reference '" + fieldName + "' in protocol tag '" + pc.name().getname() + "' in protocol '" + pt.name().getname() + "'.", false, 0000);
	...
	if (returnType instanceof PrimitiveType) {
		...
		Error.error(rs, "Procedure return type is void; return statement cannot return a value.", false, 3040);
	    if (!pt.isVoidType() && rs.expr() == null)
		Error.error(rs, "Procedure return type is `" + pt + "' but procedure return type is void.", false, 3041);
	    if (pt.isVoidType() && rs.expr() == null)
		return null;
	}
	...
	if (!Type.assignmentCompatible(returnType, eType)) 
	    Error.error(rs, "Incompatible type in return statement.", false, 3042);
	...
	if ((!eType.isIntegralType() && !eType.isProtocolType()) || eType.isLongType())
	    Error.error(ss, "Switch statement expects value of type int or a protocol tag.", false, 3043);
	...
	if (eType.isProtocolType()) {
		...
		Error.error(ss.expr(), "Nested switch statements on the same protocol type is not allowed.", false ,0000);
	}
	...
	if (va.init() != null) {
		...
		Error.error(va, "Cannot assign value of type " + iType.typeName() + " to variable of type " +vType.typeName() + ".", false, 3058);
	}
	return null;
    }
