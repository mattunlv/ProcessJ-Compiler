...
public class santachunk {
			...
			while( getTrue() ) {
			  ...
			  switch( _ld0$msg.tag ){
			      ...
			      case "reindeer_ready":
			        ...
			        break;
			      case "harness":
			        std.io.println( ("Santa: just harnessing reindeer:" + ((Protocol_Santa_msg.Protocol_Santa_msg_harness) _ld0$msg).id) );
			        break;
			      case "mush_mush":
	...
	public static class santa extends PJProcess {
		...
		public synchronized void run() {
			...
			_ld0$my_seed = _pd$seed;
			while( getTrue() ) {
			  _ld11$alt = new PJAlt(2, this);	
			  Object[] guards = {_pd$from_reindeer, _pd$knock};
			  boolean[] boolGuards = {true, true};	

			  boolean bRet = _ld11$alt.setGuards(boolGuards, guards);
			  ...
			  _ld10$chosen = _ld11$alt.getReadyGuardIndex();
			  switch(_ld10$chosen) {
			  	case 0: 
	...
	public static class random_wait extends PJProcess {
		...
		public synchronized void run() {
			...
			std.Random.initRandom( _pd$seed );
