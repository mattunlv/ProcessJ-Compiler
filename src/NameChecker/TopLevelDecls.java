...
/** ToplevelDecls.java:
 *   
 *  Inserts all top-level declarations into a symbol table.
...
public class TopLevelDecls<T extends AST> extends Visitor<T> {
    ...
    /**
     * Establishes a symbol table with the top-level declarations
     * declared in the file associated with this compilation inserted.
     * Also causes the creation of a symbol table chain for imported
     * files that is available through the `importParent' field of the
     * symbol table. This chain can be traversed through its parent
     * links.
     *
     * @param co a Compilation parse tree node.
     */
    ...
    /**
     * Static class used for filtering files in imports (only the ones
     * ending in the proper extension will be considered) PJFiles
     * takes a directory and a filename and determines if it should be
     * imported - importFileExtension is ".pj" by default. This is
     * used for importing files in an import statement ending in *.
     */
    ...
    /** Given a directory, makeFileList creates an array list of
     * Strings representing the absolute paths of all the files in the
     * directory and its sub-directories that satisfy the filter in
     * the PJFiles class.
     *
     * @param list After execution list will contain the list of file
     * names in the directory given by the `directory' parameter.
     * @param directory The name of the directory from which to import
     * files.     
     */
     ...
     *
     * @param a An AST node - just jused for line number information.
	...
	try {
	    ...
	    Error.error(a, "Something went wrong while trying to parse " + fileName, true, 2105);
	}
	return null;
    }
    ...
    /**
     * visitImport will read and parse an import statement. The chain
     * of symbol tables will be left in the 'symtab' field. The
     * parentage of multiple files imported in the same import is also
	...
	if (Modifier.hasModifierSet(pd.modifiers(), Modifier.MOBILE))
		...
		Error.error(pd, "Mobile procedure '" + pd.name().getname() + "' must have void return type.",false, 2109); // TODO error number
	...
	if (s == null) { 
		...
		if (Modifier.hasModifierSet(pd.modifiers(), Modifier.MOBILE)) {
			...
			Error.error(pd,"Only one declaration of mobile procedure '" + pd.name().getname() + "' may exist.", false, 2110); 
		    else
			Error.error(pd,"Non-mobile proecdure '" + pd.name().getname() + "' already exists.", false, 2111); 
		} else   			
		    st.put(pd.signature(), pd);
	    } else
	...
	if (!symtab.put(nt.name().getname(), nt))
	    Error.error(nt, "Type with name '" + nt.name().getname() + "' already declared in this scope.", false, 0000);
	return null;
    }
