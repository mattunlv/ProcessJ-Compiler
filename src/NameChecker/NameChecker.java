...
public class NameChecker<T extends Object> extends Visitor<T> {
	...
	public T visitInvocation(Invocation in) {
		...
		if (in.target() == null) {
			...
			if (o == null)
				Error.error(in, "Procedure '"  + in.procedureName().getname() + "' not found", false, 2207);
			if (!(o instanceof SymbolTable))
				Error.error(in, "Cannot invoke non-procedure '" + in.procedureName().getname() + "'.", false, 2208);
			else
	...
	public 	T visitNamedType(NamedType nt) {
		...
		Error.error(nt,"Symbol '" + nt.name().getname() + "' is not a type.", false, 2209);
	    }
	...
	public T visitNameExpr(NameExpr ne) {
		...
		if (o == null)
			Error.error(ne,"Symbol '" + ne.name().getname() + "' not found.", false, 2210);
		else
			ne.myDecl = (AST)o;
		return null;
	}
	...
	public T visitNewMobile(NewMobile nm) {
		...
		if (o == null)
			Error.error(nm,"Symbol '" + nm.name().getname() + "' not found.", false, 2211);
		else
	...
	public T visitProcTypeDecl(ProcTypeDecl pd) {
		...
		for (Name name : pd.implement()) {
			...
			if (o == null) 
			    Error.error(pd, "Symbol '" + name.getname() + "' not found.", false, 2212);
			...
			// procedures with the same name.
			    ...
			    Error.error(pd, "Symbol '" + name.getname() + "' not a procedure.", false, 2213);
		}
		if (pd.body() != null)
			pd.body().visit(this);
		currentScope = currentScope.closeScope();
	...
	public T visitProtocolLiteral(ProtocolLiteral pl) {
		...
		if (o == null) 
		    Error.error(pl, "Symbol '" + pl.name().getname() + "' not found.", false, 2214);
		else if (!(o instanceof ProtocolTypeDecl))
		    Error.error(pl, "Symbol '" + pl.name().getname() + "' is not a protocol type.", false, 2215);
		...
		Error.error(pl,"Undefined protocol tag name '" + pl.tag().getname() + "' in literal of protocol type '" + pl.name().getname() + "'.", false, 2216);
	...
	public T visitRecordLiteral(RecordLiteral rl) {
		...
		if (o == null)
		    Error.error(rl,"Symbol '" + rl.name().getname() + "' not found.", false, 2219);
		else if (!(o instanceof RecordTypeDecl))
		    Error.error(rl, "Symbol '" + rl.name().getname() + "' is not a record type.", false, 2220);
		else 
	...
	public T visitRecordTypeDecl(RecordTypeDecl rt) {	
		...
		for (Name n : rt.extend()) {
			Object o = resolveName(n);
			if (o == null)
			    Error.error(n,"Symbol '" + n.getname() + "' not found.", false, 2221);
			else if (!(o instanceof RecordTypeDecl))
			    Error.error(n, "'" + n.getname() + "' cannot be extended as a record as it is not of record type.", false, 2222);
		}
		...
		for (Name name : rt.extend()) {
			if (hs.contains(name.getname()))
			    Error.error(rt,"'" + name.getname() + "' repeated in extends clause of record type '" +rt.name().getname() + "'.", false, 2223);
			hs.add(name.getname());
		}
		rt.body().visit(this);
