...
public class CodeGeneratorJava<T extends Object> extends Visitor<T> {
    ...
    public T visitAltStat(AltStat as) {
        ...
        for (int i = 0; i < caseCount; i++) {
            ...
            if (ac.precondition() == null) {
                ...
                tempExprs.add((String) new LocalDecl(new PrimitiveType(
                        PrimitiveType.BooleanKind), new Var(n, ac
    ...
    public T visitChannelWriteStat(ChannelWriteStat cw) {
        ...
        if (myType.isChannelEndType()) {
            ...
            if (chanType.shared() == ChannelType.SHARED_WRITE
                    || chanType.shared() == ChannelType.SHARED_READ_WRITE) {
    ...
    public T visitCompilation(Compilation c) {
         ...
         */
        Collections.sort((List<Type>) extendedDecls.children,
                new Comparator<Type>() {
                    @Override
                    public int compare(Type o1, Type o2) {
                        return ((RecordTypeDecl) o1).extend().size()
                                - ((RecordTypeDecl) o2).extend().size();
                    }
                });
        typeDeclsStr.addAll(Arrays.asList((String[]) individualDecls
                .visit(this)));
        typeDeclsStr
                .addAll(Arrays.asList((String[]) extendedDecls.visit(this)));
        ...
        Collections.sort((List<Type>) extendedDecls.children,
                new Comparator<Type>() {
                    @Override
                    public int compare(Type o1, Type o2) {
                        return ((ProtocolTypeDecl) o1).extend().size()
                                - ((ProtocolTypeDecl) o2).extend().size();
                    }
                });
        ...
        if (this.addTrueMethod) {
            typeDeclsStr.add((String) (((ST) group.getInstanceOf("GetTrue"))
                    .render()));
        }
    ...
    public T visitClaimStat(ClaimStat cs) {
        ...
        for (int k = 0; k < claimExprs.size(); k++) {
            ...
            if (ast instanceof LocalDecl) {
                ...
                chanNameToEndType.put(ld.var().name().getname(),
                        isReadingChannelEnd);
            } else {
                chanNameToEndType.put((String) ast.visit(this),
                        isReadingChannelEnd);
            }
    ...
    public T createNewArray(String left, NewArray ne) {
        ...
        if (bt.isChannelType() || bt.isChannelEndType()
                || (bt.isNamedType() && (isProtocolType((NamedType) bt)))) {
    ...
    public T createInvocation(String left, Invocation in,
            boolean isParamInvocation) {
        return createInvocation(left, in, null, isParamInvocation, -1);
    }

    public T createInvocation(String left, Invocation in,
            List<String> barriers, boolean isParamInvocation,
            int invocationWrapper) {
        Log.log(in.line + ": Creating Invocation ("
                + in.procedureName().getname() + ") with LHS as " + left);
         ...
         */
        if (yields && invocationWrapper != INV_WRAP_PAR
                && invocationWrapper != INV_WRAP_PARFOR) {
            return (new ParBlock(new Sequence(new ExprStat(in)), new Sequence()))
                    .visit(this);
        }
        ...
        for (int i = 0; i < params.size(); i++) {
            ...
            if (e != null) {
                if (e instanceof ChannelReadExpr) {
                    ...
                    String typeString = in.targetProc.formalParams().child(i)
                            .type().typeName();
                    String tempVar = globalize("temp", false);
                    _gLocals.add(typeString + " " + tempVar);
                    String chanReadBlock = (String) createChannelReadExpr(
                            tempVar, (ChannelReadExpr) e);
                    ...
                    String typeString = ((Invocation) e).targetProc
                            .returnType().typeName();
                    String tempVar = globalize("temp", false);
                    _gLocals.add(typeString + " " + tempVar);
                    String invocationBlock = (String) createInvocation(tempVar,
                            (Invocation) e, true);
    ...
    public T createChannelReadExpr(String left, ChannelReadExpr cr) {
        ...
        if (myType.isTimerType()) {
            ...
            template.add("alt",
                    (State.is(State.ALT) && State.is(State.ALT_GUARD)));
        ...
        if (fs.isPar()) {
            ...
            if (fs.stats() instanceof Block) {
                ...
                if (b.stats().size() == 1
                    ...
                    Invocation in = (Invocation) ((ExprStat) b.stats().child(0))
                            .expr();
                    rendered = (String) createInvocation(null, in, barriers,
                            false, INV_WRAP_PARFOR);
                } else {
                    rendered = (String) (new ProcTypeDecl(new Sequence(), null,
                            new Name("Anonymous"), new Sequence(),
                ...
                if (es.expr() instanceof Invocation) {
                    rendered = (String) createInvocation(null,
                            (Invocation) es.expr(), barriers, false,
                            INV_WRAP_PARFOR);
                } else {
                    rendered = (String) (new ProcTypeDecl(new Sequence(), null,
                            new Name("Anonymous"), new Sequence(),
                            new Sequence(), a, new Block(new Sequence(
                                    fs.stats())))).visit(this);
                }
            } else {
                rendered = (String) (new ProcTypeDecl(new Sequence(), null,
                        new Name("Anonymous"), new Sequence(), new Sequence(),
                        a, new Block(new Sequence(fs.stats())))).visit(this);
            }
    ...
    public T visitLocalDecl(LocalDecl ld) {
        ...
        if (ld.type() instanceof ChannelEndType
                || (ld.type() instanceof ArrayType && ((ArrayType) ld.type())
                        .baseType() instanceof ChannelEndType)) {
        ...
        if (State.is(State.PROC_YIELDS)
                && (right instanceof ChannelReadExpr
                        || right instanceof Invocation || right instanceof NewArray)) {
            String assignment = (String) new Assignment(new NameExpr(ld.var()
                    .name()), right, Assignment.EQ).visit(this);
            return (T) assignment;
        } else {
            /*
             * Resolve type. 
    ...
    public T visitParamDecl(ParamDecl pd) {
        ...
        if (pd.type() instanceof ChannelEndType
                || (pd.type() instanceof ArrayType && ((ArrayType) pd.type())
                        .baseType() instanceof ChannelEndType)) {
            _channelEndMap.put(name, endType);
        }
        ...
        for (int k = 0; k < se.size(); k++) {
            ...
            if (st != null) {
                if (st instanceof ExprStat
                        && ((ExprStat) st).expr() instanceof Invocation) {

                    ExprStat es = (ExprStat) st;

                    stats[k] = (String) createInvocation(null,
                            (Invocation) es.expr(), barriers, false,
                            INV_WRAP_PAR);
                    ...
                    Annotations a = new Annotations();
                    a.add("yield", "true");
                    stats[k] = (String) (new ProcTypeDecl(new Sequence(), null,
                            new Name("Anonymous"), new Sequence(),
                            new Sequence(), a, new Block(new Sequence(st)))
                            .visit(this));
                }
            }
        }
    ...
    public T visitProcTypeDecl(ProcTypeDecl pd) {
        ...
        if (anonymous) {
            ...
            if (pd.body().stats().size() > 0) {
                lastStat = pd.body().stats()
                        .child(pd.body().stats().size() - 1);
            }
            ...
            boolean isMain = "main".equals(_currentProcName)
                    && MAIN_SIGNATURE.equals(pd.signature());
    ...
    public T visitRecordAccess(RecordAccess ra) {
        ...
        if (tType.isRecordType()) {
            ...
            String caseName = _gProtoTagNameMap.get(_protocolTagsSwitchedOn
                    .get(pt.name().getname()));
    ...
    public T visitSwitchStat(SwitchStat st) {
        ...
        boolean oldVal = State.set(State.PROTOCOL_EXPR,
                st.expr().type.isProtocolType());
        ...
        if (State.is(State.PROTOCOL_EXPR)) {
            ...
            for (SwitchGroup sg : st.switchBlocks()) {
                ...
                _protocolTagsSwitchedOn.put(pt.name().getname(),
            ...
            switchGroups = Arrays.asList((String[]) st.switchBlocks().visit(
                    this));
        }
    ...
    public ProtocolCase findProtocolTag(ProtocolTypeDecl pt,
    ...
    public String getQualifiedPkg(ProcTypeDecl pd) {
        ...
        if (myPkg.contains(this.originalFilename)) { //invocation from the same file
            ...
            String includeDirPath = Settings.includeDir + File.separator
                    + Settings.targetLanguage + File.separator;
    ...
    private void writeToFile(String finalOutput, String filename) {
        ...
        try {
            ...
            String javafile = dir.getAbsolutePath() + File.separator + filename
                    + ".java";
